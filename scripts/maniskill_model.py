...
with open('configs/dataset_stat.json','r') as f:
    data=json.load(f)
    DATA_STAT={'state_min':data['my_cool_dataset']['state_min'],
               'state_max':data['my_cool_dataset']['state_max'],
               'action_min':data['my_cool_dataset']['state_min'],
               'action_max':data['my_cool_dataset']['state_max']}
...
class RoboticDiffusionTransformerModel(object):
    ...
    def _format_joint_to_state(self, joints):
        ...
        # normalize to -1,1
        # joints = (joints - self.state_min) / (self.state_max - self.state_min) 
        B, N, _ = joints.shape
        # state = torch.zeros(
